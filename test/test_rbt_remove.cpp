...
TEST(RedBlackTreeRemove, Remove_node_scenario_1)
{
    ...
    {
        node_to_remove->value = values_to_remove[i];
        my_tree->rbt_node_remove(node_to_remove);

        status = my_debug->test_red_black_tree(my_tree);
        if (status == false)
        {
            std::cout<<"La violacion ocurrió removiendo: "<<values_to_remove[i]<<std::endl;
            break;
        }
...
TEST(RedBlackTreeRemove, Remove_node_scenario_5)
{
    ...
    for (int i = 0; i < values_to_remove.size(); i++)
    {
        ...
        if (status == false)
        {
            std::cout<<"La violacion ocurrió removiendo el valor: "<<values_to_remove[i]<<std::endl;
            break;
        }
